...
export function attachSocket(server, { corsOrigin }) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('joinRoom', async ({ boardId }) => {
            ...
            // Add user to active users list (deduplicate by userId to handle refresh)
            if (!activeUsers.has(String(boardId))) {
                activeUsers.set(String(boardId), []);
            }
            let boardUsers = activeUsers.get(String(boardId));
            // Remove any stale entries for this user (e.g. from a previous socket before disconnect fires)
            boardUsers = boardUsers.filter(u => u.userId !== user.id);
            boardUsers.push({ socketId: socket.id, userId: user.id, name: user.name });
            activeUsers.set(String(boardId), boardUsers);
